...
var MasterSchedule = /** @class */ (function () {
    ...
    MasterSchedule.prototype.OnDateChangeEvent = function (v) {
        ...
        this.scheduleService.getRoomSchedule({ FDate: v, branches: this.selectedBranch.map(function (x) { return x.branchId; }) }).subscribe(function (data) {
            var e_1, _a;
            console.log(data, "room data");
            _this.rt = data[0];
            //console.log("room",this.rt);
            //console.log("rt.DATE",this.rt.DATE)
            var rtNewDate = new Date(_this.rt.DATE);
            _this.rtFormat = _this.datePipe.transform(rtNewDate, "yyyy-MM-dd");
            _this.rtFormatNepali = _this.masterRepo.toBSDate(_this.rtFormat);
            _this.roomtimeTableMaker();
            try {
                for (var _b = tslib__WEBPACK_IMPORTED_MODULE_0__["__values"](_this.rt.ROOMS), _c = _b.next(); !_c.done; _c = _b.next()) {
                    var r = _c.value;
                    var bt = r.BOOKEDTIMES;
                    //console.log("before",r)
                    r.BOOKEDTIMES = bt.sort(function (x, y) { return new Date(x.STARTDATETIME).getTime() - new Date(y.STARTDATETIME).getTime(); });
                    //console.log("after",r);
                }
            }
            catch (e_1_1) { e_1 = { error: e_1_1 }; }
...
var TreatmentEntry = /** @class */ (function () {
    ...
    TreatmentEntry.prototype.onImageClick = function () {
    ...
    TreatmentEntry.prototype.SaveClickEvent = function () {
        ...
        if (this.disease && this.disease !== "") {
            this.showMessage("Please add pending Disease", -1);
            return;
        }
        if (this.medication && this.medication !== "") {
            this.showMessage("Please add pending Medication", -1);
            return;
        }
        if (this.prescription && this.prescription !== "") {
            this.showMessage("Please add pending Morning Prescription", -1);
            return;
        }
        if (this.eveningPrescription && this.eveningPrescription !== "") {
            this.showMessage("Please add pending Evening Prescription", -1);
            return;
        }
        if (!this.model.Healer || !this.model.Healer.NAME || this.model.Healer.NAME === "") {
            this.showMessage("Staff is not selected. Please select a staff and try again", -1);
            return;
        }
        this.showMessage("Saving Data please wait...", -1);
        this.onsubmit();
    };
    TreatmentEntry.prototype.saveChildModal = function () {
        this.showMessage("Updating Data please wait...", -1);
    };
        ...
        try {
            ...
            var sub = this.masterService.postform(this.mode, this.model, "/saveTreatment", this.uploadFiles)
                ...
                if (data.status == 'ok') {
                    ...
                    //Displaying dialog message for save with timer of 1 secs
                    _this.showMessage("Data Saved Successfully", 1000, function () {
                    ...
                    _this.showMessage("Error in Saving Data:" + data.result._body, 3000);
                }
            }, function (error) { alert(error); });
            this.subcriptions.push(sub);
        }
    ...
    TreatmentEntry.prototype.DeleteEvent = function () {
        ...
        var sub = this.masterService.postmaster('delete', this.model, "/DeleteSchedule")
            ...
            //console.log(data);
            if (data.status == 'ok') {
                _this.showMessage("Delete Successfull...", 2000);
                _this.router.navigate([_this.returnUrl]);
            }
            else {
                //the ConnectionString in the server is not initialized means the the token 's user is not int the list of database user so it could't make connectionstring. Re authorization is requierd
            ...
            if (e.key == "Enter") {
                ...
                if (!this.model.Diseases.find(function (x) { return x == e.target.value; })) {
                    ...
                    if (disease) {
                        ...
                        this.showMessage("Disease not found in the list", -1);
                        return;
                    }
                }
                e.target.value = "";
    ...
    TreatmentEntry.prototype.addMedication = function (e) {
        if (e.target.value !== "") {
            ...
            if (!this.model.Medications.find(function (x) { return x.DESCRIPTION == e.target.value; })) {
                var medication = this.medicationList.find(function (x) { return x.DESCRIPTION == e.target.value; });
                if (medication) {
                    this.model.Medications.push(medication);
                }
                else {
                    this.showMessage("Medication not found in the list", -1);
                    return;
                }
            }
            e.target.value = "";
    ...
    TreatmentEntry.prototype.addPrescription = function (e, Prescriptions) {
        if (e.target.value !== "") {
            ...
            if (!Prescriptions.find(function (x) { return x.DESCA == e.target.value; })) {
                var prescription = this.prescriptionList.find(function (x) { return x.DESCA == e.target.value; });
                if (prescription) {
                    Prescriptions.push(prescription);
                }
                else {
                    this.showMessage("Prescription not found in the list", -1);
                    return;
                }
            }
            e.target.value = "";
    ...
    TreatmentEntry.prototype.showMessage = function (message, closeAfter, onClose) {
        var _this = this;
        if (closeAfter === void 0) { closeAfter = 3000; }
        this.DialogMessage = message;
        if (!this.childModal.isShown)
...
//# sourceMappingURL=schedule-schedule-module.bcce63bac700e3810eca.js.map